...
class AILoopEngine:
    ...
    def _build_system_prompt(self, context: PlannerContext) -> str:
        ...
        prompt += (
            ...
            "3. If the user provided a rich, detailed, or self-contained prompt, DO NOT ask unnecessary questions. Proactively design the architecture and propose concrete work items.\n"
            "4. Leave 'clarifications' as an EMPTY list [] unless there is a critical ambiguity or high-risk architectural conflict that genuinely blocks execution.\n"
            "5. When you have finished gathering knowledge and want to propose the final execution plan, output a raw JSON object matching the PlanningResult schema in your content message (with work_items, acceptance_criteria, risk_analysis). Do NOT call any tools when you output the final JSON plan.\n"
            "6. NEVER output markdown code blocks around the final JSON plan if possible.\n"
